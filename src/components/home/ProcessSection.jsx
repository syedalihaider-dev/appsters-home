...
export default function ProcessSection({ data }) {
  ...
  const subHeading = data?.subHeading || "8 Steps. 1 Powerful Outcome.";
  const title = data?.title || 'A <span class="primarytxt">Battle-Tested</span> App Development <span class="primarytxt">Process.</span>';
  ...
  const activeIndex = active < steps.length ? active : 0;
    ...
    <section className={styles.processSection}>
      ...
      <div className={styles.sec_content}>
            ...
            <div className="col-sm-12 col-md-6">
              <div className={styles.sec_right}>
                ...
                <div className={styles.content_wrapper}>
                  ...
                  >
                    <p className={styles.step}>{currentStep.step || String(activeIndex + 1).padStart(2, '0')}</p>
                    <h5 className={styles.title}>{currentStep.title}</h5>
                    <p className={styles.para}>{currentStep.text || currentStep.desc}</p>
        ...
        <div className={styles.dots_wrapper}>
          {steps.map((_, i) => (
            <div key={i} className={`${styles.dot} ${activeIndex === i ? styles.activeDot : ""}`} onClick={() => setActive(i)}
            ></div>
          ))}
        </div>
      </div>
